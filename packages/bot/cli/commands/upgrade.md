...
Rules:
- Do NOT re-implement the upgrade steps — the tool re-runs `bun . up`, which handles git pull, `bun install`, and the service restart.
...
- The user already sees `⏳ Upgrading OpenKitten…` before the restart and `✅ OpenKitten upgraded` after it. No further acknowledgement from you is needed.
