...
it("renders the email verification template with a custom URL", async () => {
  const html = await render(
    EmailVerification({ url: "https://www.openkitten.com/verify?token=abc" }),
  );
  ...
  expect(normalizedHTML).toContain(
    "https://www.openkitten.com/verify?token=abc",
  );
...
it("falls back to the website URL in the email verification template", async () => {
  ...
  expect(normalizedHTML).toContain("https://www.openkitten.com");
  expect(normalizedHTML).toContain(
...
it("renders the password reset template with a custom URL", async () => {
  const html = await render(
    PasswordReset({ url: "https://www.openkitten.com/reset?token=xyz" }),
  );
  const normalizedHTML = html.replaceAll("<!-- -->", "");
  ...
  expect(normalizedHTML).toContain(
    "https://www.openkitten.com/reset?token=xyz",
  );
